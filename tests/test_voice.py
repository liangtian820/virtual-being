...
def test_tts_synthesize_writes_audio(tmp_path) -> None:
    """TTS 应生成非空音频文件并记录耗时。"""
    tts = EdgeTTS(tts_cls=FakeCommunicate)
...
def test_tts_failure_raises_runtime_error(tmp_path) -> None:
    ...
    tts = EdgeTTS(tts_cls=BoomCommunicate)
...
def test_pipeline_end_to_end(tmp_path) -> None:
    ...
    fake_tts = EdgeTTS(tts_cls=FakeCommunicate)
...
def test_pipeline_empty_transcript_raises(tmp_path) -> None:
    ...
    pipe = VoicePipeline(
        asr=fake_asr, tts=EdgeTTS(tts_cls=FakeCommunicate), agent=FakeAgent(), reply_dir=str(tmp_path / "r")
    )
