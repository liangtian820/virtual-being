...
class Config:
    ...
    voice_llm_model: Optional[str] = field(
        default_factory=lambda: os.getenv("VOICE_LLM_MODEL") or "llama3.2:3b"
    )
    ...
    # M4.3/M4.4：TTS 后端（默认 piper=本地离线稳定（用户决策 <5s 优先）；edge_tts=在线晓晓音质好/需联网，
    # 且实测多次断连。可设 TTS_BACKEND=edge_tts 切回）
    tts_backend: str = field(default_factory=lambda: os.getenv("TTS_BACKEND", "piper"))
    # M4 回复音频落盘目录（相对项目根）
