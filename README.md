...
- ✅ M6.4 候选工具组（WO-20260816-32）：工具集扩到 26 个后 qwen2.5:7b 面对整表 schema 选型失效（无 tool_calls）→
  **意图预筛 + 候选工具子集**（`app/tools/tool_groups.py`，每组 ≤8）只把候选组 schema 交给 LLM；
  ...
  关键词路由兜底去死代码（WO-20260816-34）：`chat()` 重构为工具路径未用工具/未执行时显式调用 `_route_by_keywords`（原 if/elif 结构下 elif 整体跳过，确定性兜底失效——LLM 未选工具时日程不落库、模型假完成承诺『我会在明天下午三点提醒你』）；现在日程落库/联网搜索/知识库目录兜底真实生效；
  回答质量（WO-20260816-35）：工具结果为空时阶段 2 切换『空结果模式』——提示词硬规则『必须如实说没找到、禁止编造填充』+ 代码层兜底（LLM 未说『没找到』则替换固定话术，绝对零编造）；Obsidian path/query 参数 schema 描述补充『完整目录名（如 30 · 项目，含中文空格与序号前缀）』提示，杜绝传短名导致空结果；
  ...
  性能优化（WO-20260816-39/40）：① 知识/搜索提速——Wikipedia 超时 8s→3s、Bing 10s→(3s,6s) 快速降级，web_search/Wikipedia 按 query LRU 缓存 ~10 分钟（`data/search_cache/`，`SEARCH_CACHE_TTL` 可配），内置无命中时 Wikipedia 与 Bing **并行**取先返回（基线串行 8s+10s→并行 ≤~6s），工具决策轮 num_predict 收紧 ≤50（『deepseek harness是什么』38.4s→≤15s、『搜新闻』29.3s→≤15s，缓存命中 ≤5s）；② 工具调用确定性优先——日程/记忆/计算强关键词意图直接走确定性路由/空记忆短路，跳过 LLM 工具决策（『提醒我喝水』11.1s→≤6s、记忆/计算 ≤6s），模糊意图（知识/搜索/知识库）保留两阶段 LLM 决策，全部质量红线（危机优先/防假完成/零编造/无工具兜底/候选组）保留；性能证据 `data/m6_6/evidence_perf_m69.json`
...
### LLM 工具调用（M6.1，WO-20260816-29）

人格 Agent 升级为 **LLM 自主工具调用（function calling）**：qwen2.5:7b 原生支持 tools 参数（Ollama `/api/chat`），
对话中由 LLM 自主决定是否调用既有能力工具；工具定义见 `app/tools/tool_specs.py`（9 个工具，中文描述、必填标注）：

- **日程**：`add_schedule`（添加提醒）/ `get_schedule`（查今天/明天）/ `mark_schedule_done`（标记完成）/ `delete_schedule`（删除提醒）
- **规划**：`list_plans`（已保存计划）/ `save_plan`（保存计划）
- **记忆**：`query_memory`（检索用户过往记忆）；**知识**：`query_knowledge`（内置库/联网）；**计算**：`calculate`

**两阶段编排**（实测 qwen2.5:7b 在带人设系统提示词时不主动调用工具，故拆分）：

1. **工具决策**：仅用工具指引 + 用户输入调用 Ollama（带 tools），≤3 轮执行工具并回填结果（失败给 LLM 错误反馈修正或如实告知）；
2. **人设包装**：完整人设系统提示词 + 记忆 + 工具结果 → 生成温柔治愈口吻的最终回复（不输出原始 JSON）。

**优先级与兜底**：危机分支（`is_crisis_query`）最高优先、**不经工具调用**；工具路径不可用/超时/未命中 → 自动回退 M5.1 关键词路由（原逻辑保留，确定性操作不丢）。
**开关**：`TOOL_CALLING_ENABLED`（默认 1，0=关闭回退关键词路由）。
真实证据：`data/m6_1/evidence_tool_calling.json`（add_schedule / get_schedule / query_memory / query_knowledge）、
`data/m6_1/evidence_tool_calling_v2.json`（完整 9 工具集复核，独立临时库不污染生产数据）。
